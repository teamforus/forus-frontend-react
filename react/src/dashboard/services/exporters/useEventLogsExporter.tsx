...
import { useEventLogService } from '../EventLogService';

export default function useEventLogsExporter() {
    ...
    const exportData = useCallback(
        (organization_id: number, filters: object = {}) => {
            ...
            eventLogService.exportFields(organization_id).then((res) => {
                openModal((modal) => (
                    <ModalExportDataSelect modal={modal} sections={makeSections(res.data.data)} onSuccess={onSuccess} />
                ));
            });
        },
        [makeSections, openModal, pushApiError, saveExportedData, setProgress, eventLogService],
    );
