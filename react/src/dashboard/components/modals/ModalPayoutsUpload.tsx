...
export default function ModalPayoutsUpload({
    ...
    const validateCsvData = useCallback(
        (data: Array<RowDataProp>) => {
            ...
            const invalidPerVoucherAmount = data.filter((row: RowDataProp) => {
                return row.amount > parseFloat(fund.limit_per_voucher);
            });
    ...
    const uploadToServer = useCallback(async () => {
        ...
        if (data.length > 0) {
            const isValid = await startUploading(data, true);
            console.log('isValid', isValid);

            if (isValid) {
                const isUploaded = await startUploading(data, false);
